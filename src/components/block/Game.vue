	...
	<div class="game">
		<table class="board" id="target">
          ...
          <span v-if="Target['i'+i+'-j'+j]">
            <Castle v-if="Target['i'+i+'-j'+j]['type']=='castle'" :id="Target['i'+i+'-j'+j]['tokenID']" :castleID="Target['i'+i+'-j'+j]['playerCastle']" :castleNum="Target['i'+i+'-j'+j]['castleNum']" :class="{active: myTurn && Target['i'+i+'-j'+j]['playerCastle']==myCastleID}" @click="takeCastleToken($event,Target['i'+i+'-j'+j]['tokenID'],Target['i'+i+'-j'+j]['playerCastle'],Target['i'+i+'-j'+j]['castleNum'])"/>
		...
		<div class="right-column">
			<div class="game-token">
				<div class="castle">
					<Castle v-for="(val,key) in castlePool" v-show="!val.isUsed" :key="key" :id="key" :castleID="myCastleID" :castleNum="val.castleNum" :class="{active: myTurn && !isPutToken}" @click="takeCastleToken($event,key,myCastleID,val.castleNum)"/>
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			takeCastleToken(event,token,myPlayerCastle,castleNum) {
				event.stopPropagation();
				if(!this.setTakeToken(token)) return;
				//不是拿自己的城堡
				/*var regex = new RegExp("castle"+this.myCastleID, 'g');
				if(!regex.test(token)) return;*/
				if(myPlayerCastle != this.myCastleID) return;

				this.isTakeToken = true;
				this.myTakeToken = token;
				this.tokenType = 'castle';
				this.myPlayerCastle = myPlayerCastle;
				this.myCastleNum = castleNum;
			},
			takeScoreToken(val) {
				if(!this.setTakeToken(val.token)) return;
